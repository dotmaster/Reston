...
var request_object = function(method, address, headers){
	...
	if(http.createClient){
		...
		cl.on('error', function(err){debugger; console.log('err '+err)})
		var req = cl.request( method, (urlinfo.pathname || '/') + ((urlinfo.query && '?') || '') + (urlinfo.query || ''), core.mixin({
				'Host': urlinfo.host
			}, headers));
		req.on('error', function(err){debugger; console.log('err '+err)})
		return req;
	}
};
...
core.mixin( proto, {
	...
	sendMultiPart: function(data){
		...
		//this.headers['Content-Encoding'] = 'chunked'
		...
		mw.writebody(req, function(){
		  debugger
		  //req.end(); //did comment that out at first		  
		});
