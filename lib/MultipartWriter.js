...
function file_data_prefix(boundary, name, filename, filetype, filesize){
	var ret=get_boundary(boundary)+CRLF+
...
function write_part(writer, name, value, boundary, write_complete, write_error){
	if(value instanceof FileStream){
		writer.write(file_data_prefix(boundary, name, value.basename(), value.filetype(), value.filesize()), "binary");
